...
auto
Patches::PatchHealthBar(int32_t ver_offset) -> bool
{
  uintptr_t address = 0;
...
auto
Patches::StartOHKOThread(int32_t ver_offset, bool die_after_bike_fall) -> bool
{
...
Patches::OHKOThread(LPVOID param) -> DWORD
{
  ...
  uintptr_t max_health_address = 0;
  uintptr_t max_armour_address = 0;
  ...
  while (true) {
    auto player = Memory::ReadMemory<uintptr_t>(player_pointer);
    ...
    static constexpr uintptr_t k_health_offset = 0x540;
    auto health_address = uintptr_t{ player + k_health_offset };

    if (args.die_after_bike_fall) {
      static constexpr uintptr_t k_vehicle_model_id_offset = 0x22;
      static constexpr float k_crash_hp_delta = 0.15f;
      static uintptr_t last_vehicle = 0;
      static float last_health = 1.0001f;
      auto current_vehicle = Memory::ReadMemory<uintptr_t>(0x00BA18FC);
    ...
    auto armour_address = uintptr_t{ player + 0x548 };
